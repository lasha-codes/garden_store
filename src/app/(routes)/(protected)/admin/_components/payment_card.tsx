...
const PaymentCard = ({
  ...
  return (
    <div className='w-full flex items-center justify-between py-3 border-b-2 border-[#202020] last:border-none px-3'>
      <div className='flex items-center gap-3'>
        ...
        <span className='text-[#CACACA] text-[15px]'>{paymentId}</span>
      </div>
      <div className='flex items-center gap-3'>
        <div
          className={`${
            status.includes('complete') || status === 'succeeded'
              ? 'bg-main/60 px-3.5 py-1 rounded-[10px] text-main font-semibold capitalize'
              : 'bg-red-500/60 px-3.5 py-1 rounded-[10px] text-red-500 font-semibold capitalize'
          }`}
        >
          {status}
        </div>
        <Link
          className='text-[#9898ee]'
          target='_blank'
